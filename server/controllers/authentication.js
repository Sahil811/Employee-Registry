 ...
 * @param  Authorization as API KEY: VALUE in postman
 */
const prepareDecodedData = ({ authorization, type }) =>
 ...
 */
const commonDecodingHandler = ({ req, res, next, type }) => {
  const {
  ...
  if (authorization) {
    prepareDecodedData({ authorization, type })
      ...
      .catch(() =>
        res.status(401).send({
          code: 401,
          message: "Token might be invalid or has been expired",
          error: "Token Invalid.",
        })
      );
  } else {
