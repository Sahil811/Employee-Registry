...
export default ({ userRef, action }) =>
  new Promise(async (resolve, reject) => {
    try {
      if (!(userRef && action)) {
        return reject(
          ResponseUtility.MISSING_PROPS({
            message: `Missing Property ${userRef ? "action" : "userRef"}`,
          })
        );
      }
      if (!Object.keys(ADMIN_USER_ACTIONS)[action - 1]) {
        return reject(
          ResponseUtility.GENERIC_ERR({ message: "Invalid action provided!" })
        );
      }
      await UserModel.updateOne(
        ...
        action === ADMIN_USER_ACTIONS.DELETED
          ? { deleted: true }
          : action === ADMIN_USER_ACTIONS.BLOCKED
          ? { blocked: true }
      ...
      return resolve(
        ResponseUtility.SUCCESS({
          message: `User has been successfully ${Object.keys(
            ADMIN_USER_ACTIONS
